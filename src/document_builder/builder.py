...
class DocumentBuilder:
    ...
    def build_form_pf_workpaper(
        ...
        # Section 2: Liquidity Profile (Q22)
        section_order += 1
        ...
        sections.append(DocumentSection(
            ...
            section_order=section_order,
            section_type=SectionType.TABLE,
            title="Question 22: Liquidity Profile",
            content="Portfolio liquidity by time horizon",
            column_headers=["Liquidity Bucket", "Long Value ($)", "Long % NAV", "Short Value ($)", "Short % NAV"],
            table_data=[
                [
                    bucket.get("bucket", ""),
    ...
    def build_13f_workpaper(
        ...
        sections.append(DocumentSection(
            ...
            content="Securities reportable on Form 13F",
            column_headers=["CUSIP", "Issuer", "Class", "Shares", "Value ($)", "Discretion"],
            table_data=[
                [
                    h.get("cusip", ""),
                    h.get("issuer_name", "")[:30],
                    h.get("security_class", ""),
                    f"{h.get('shares', 0):,.0f}",
                    f"${h.get('value', 0):,.0f}",
